...
def createTokenizedDataOASST(split = 'train'):
    combinedTokenizedDataset = []
    ...
    for dataset in datasets:
        ...
        for entry in ds[split]:
            if entry["lang"] == 'en':
                if entry["message_tree_id"] not in appendedEntries.keys():
                    appendedEntries[entry["message_tree_id"]] = "<|user>:\n" + entry["text"] if entry["role"] == "prompter" else "<|assistant>:\n" + entry["text"]
                else:
                    appendedEntries[entry["message_tree_id"]] += "\n<|user>:\n" + entry["text"] if entry["role"] == "prompter" else "\n<|assistant>:\n" + entry["text"]

        for k in appendedEntries.keys():
            combinedTokenizedDataset.append(tokenizer(appendedEntries[k], max_length= 511, truncation = True).input_ids + [endOfTextToken])
            totalTokens += len(combinedTokenizedDataset[-1])

    print(f"Total {split} tokens are {totalTokens}")
    
    combinedTokenizedDataset = random.sample(combinedTokenizedDataset, len(combinedTokenizedDataset))
    
    returnArray = np.zeros(totalTokens, dtype = np.uint16)
    
    i = 0
    for tokenizedEntry in combinedTokenizedDataset:
        for j in range(len(tokenizedEntry)):
            returnArray[i] = tokenizedEntry[j]
            i += 1

    return returnArray

trainData = createTokenizedDataOASST("train")
valData = createTokenizedDataOASST("validation")

# -----------------------------------------------------------------------------

out_dir = os.path.join(os.path.dirname(__file__), "Checkpoints")
max_iters = 5000 # Total number of training iterations (just a number I think will go nicely)
eval_interval = 10
log_interval = 1
eval_iters = 5
...
class GPTConfig:
    def __init__(self):
        ...
        self.n_head = 12
        self.n_embd = 768
        self.dropout = 0.1 # for pretraining 0 is good, for finetuning try 0.1+
        self.bias = False
        ...
        self.warmup_iters = 200 # how many steps to warm up for
...
ckpt_path = os.path.join(out_dir, "checkpoint - first train dataset - 136k iter.pt")
    ...
    if iter_num % eval_interval == 0:
        ...
        if losses['val'] < best_val_loss or always_save_checkpoint:
            ...
            if iter_num > 0:
                ...
                torch.save(checkpoint, os.path.join(out_dir, 'checkpoint - second train dataset - finetune.pt'))
